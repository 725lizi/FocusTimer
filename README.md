...
统计页「🧠 生成复盘脑图」基于本周真实专注数据生成可视化复盘，闭环链路：

本地专注记录 → DeepSeek 生成思维导图结构化 JSON（提示词组装、流式返回、JSON 校验与容错在 App 层完成）→ 自研开源 HAR 组件 **mindmap-harmony** 负责树形布局、渲染与缩放平移。

- 组件库保持纯 UI / 算法职责，**不内置任何 AI 与网络依赖**，安装无需 API Key；AI 能力全部位于上层 App，二者通过标准树节点 JSON 解耦。
...
- 技术文章（掘金）：[我用 ArkTS 自研了一个鸿蒙原生思维导图组件，已上架 ohpm，并让 DeepSeek 驱动它渲染复盘脑图](https://juejin.cn/post/7688341026176532516)。
